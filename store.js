...
module.exports = {
    searchHotels: function (destination) {
        return new Promise(function (resolve) {
            ...
            for (var i = 1; i <= 5; i++) {
                hotels.push({
                    name: destination + ' SearchResult ' /*+ i*/,
                    location: destination,
                    ReadersRating: Math.ceil(Math.random() * 5),
                    numberOfReviews: Math.floor(Math.random() * 5000) + 1,
                    image: 'https://placeholdit.imgix.net/~text?txtsize=35&txt=SearchResult +' + i + '&w=500&h=260'
                });
            }

            hotels.sort(function (a, b) { return b.ReadersRating- a.ReadersRating; });
